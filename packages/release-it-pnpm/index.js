...
import { cyan, dim, blue, yellow, red, bold } from 'kolorist';
import { Plugin } from 'release-it';
import semver from 'semver';

function fixChangelogTag(content) {
...
class ReleaseItPnpmPlugin extends Plugin {
    ...
    async getRecommendedVersion({ latestVersion, increment, isPreRelease, preReleaseId }) {
        ...
        if(options.snapshot) {
            ...
            const version = `0.0.0-snapshot.${timestamp}`;
        ...
        if (!options.ci) {
            const result = await versionBump({
                ...
                preid: preReleaseId,
                files: [
                    'this-is-a-non-existing-file-i-believe-you-do-not-have-it.txt'
                ]
            });
    ...
    async writeChangelog(changelog, version) {
        ...
        fs.writeFileSync(inFile, `${header + EOL + EOL}${currentChangelog}${previousChangelog ? EOL + EOL + previousChangelog.trim() : ''}${EOL}`);
        ...
        await this.step({
            task: async ()=>{
                ...
                try {
                    ...
                    this.debug('release-it-pnpm:release', {
                        config,
                        md,
                        commits
                    });
                    if (this.options['verbose']) {
                        this.log.log(cyan(config.from) + dim(' -> ') + blue(config.to) + dim(` (${commits.length} commits)`));
                        this.log.log(dim('--------------'));
                        this.log.log();
                    ...
                    if (e?.stack) this.log.error(dim(e.stack?.split('\n').slice(1).join('\n')));
                    if (webUrl) {
                        this.log.log();
...
export { ReleaseItPnpmPlugin as default };
